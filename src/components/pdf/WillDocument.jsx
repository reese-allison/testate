...
const Article = ({ title, children, articleNum }) => (
  <View style={styles.article} wrap={false}>
    <Text style={styles.articleTitle}>ARTICLE {toRoman(articleNum)} - {title}</Text>
    {children}
  </View>
)
...
const ArticleLong = ({ title, children, articleNum }) => (
  <View style={styles.article}>
    <Text style={styles.articleTitle} minPresenceAhead={50}>ARTICLE {toRoman(articleNum)} - {title}</Text>
    {children}
  </View>
)

export function WillDocument({ formData }) {
  const {
    testator,
    executor,
    children,
  ...
  const articleNums = {
    ...
    debts: articleNumber++,
    disinherit: disinheritance.include && disinheritance.persons?.length > 0 ? articleNumber++ : null,
    ...
    <Document>
      ...
      <Page size="LETTER" style={styles.page} wrap>
        ...
        <View style={{ marginBottom: 24 }}>
          ...
          <Text style={styles.subtitle}>OF {testator.fullName.toUpperCase()}</Text>
        </View>
        ...
        <Article title="FAMILY DECLARATION" articleNum={articleNums.family}>
          <Text style={styles.paragraph}>
            {testator.maritalStatus === 'married'
              ? `I am married to ${testator.spouseName}, hereinafter referred to as "my spouse."`
        ...
        <Article title="PERSONAL REPRESENTATIVE" articleNum={articleNums.representative}>
          <Text style={styles.paragraph}>
            I appoint {executor.name}
            {executor.relationship ? `, my ${executor.relationship},` : null} as the
            Personal Representative of my estate.
          </Text>
          {executor.alternateName && (
            <Text style={styles.paragraph}>
              If {executor.name} is unable or unwilling to serve, I appoint {executor.alternateName}
              {executor.alternateRelationship ? `, my ${executor.alternateRelationship},` : null} as
          ...
          <Text style={styles.listItem}>(b) Sell, lease, or mortgage any real or personal property;</Text>
          <Text style={styles.listItem}>(c) Pay all debts, taxes, and expenses of administration;</Text>
          <Text style={styles.listItem}>(d) Distribute my estate according to this Will;</Text>
          <Text style={styles.listItem}>(e) Perform all acts necessary for proper estate administration.</Text>
        </Article>
        ...
        {articleNums.guardian && (
          <Article title="GUARDIAN OF MINOR CHILDREN" articleNum={articleNums.guardian}>
            <Text style={styles.paragraph}>
              If at my death I have any minor children and their other parent is unable
              or unwilling to care for them, I appoint {guardian.name}
              {guardian.relationship ? `, ${guardian.relationship},` : null} as guardian
              of the person of my minor children.
            </Text>
            {guardian.alternateName && (
              <Text style={styles.paragraph}>
                If {guardian.name} is unable or unwilling to serve, I appoint {guardian.alternateName}
              ...
              <View key={i} style={{ marginBottom: 8 }} wrap={false}>
                ...
                {gift.alternativeBeneficiary && (
                  <Text style={[styles.paragraph, styles.indent]}>
                    If {gift.beneficiary} does not survive me, this gift shall pass to {gift.alternativeBeneficiary}.
                  </Text>
                )}
                {gift.conditions && (
                  <Text style={[styles.paragraph, styles.indent]}>
            ...
            {realProperty.items.map((property, i) => (
              <View key={i} style={{ marginBottom: 8 }} wrap={false}>
                <Text style={styles.paragraph}>
                  ...
                  {property.description ? ` (${property.description})` : null} shall
                  pass to {property.beneficiary}.
                </Text>
        ...
        <Article title="RESIDUARY ESTATE" articleNum={articleNums.residuary}>
          <Text style={styles.paragraph}>
            I give all the rest, residue, and remainder of my estate, whether real,
            personal, or mixed, and wherever situated, including any property over which
            I may have power of appointment (hereinafter referred to as my "residuary
            estate"), as follows:
          </Text>
          {residuaryEstate.distributionType === 'spouse' && testator.maritalStatus === 'married' && (
            <Text style={styles.paragraph}>
          ...
          {residuaryEstate.distributionType === 'split' && testator.maritalStatus === 'married' && (
            <Text style={styles.paragraph}>
              {residuaryEstate.spouseShare}% to my spouse, {testator.spouseName}, and {residuaryEstate.childrenShare}% to my children, in equal shares{residuaryEstate.perStirpes ? ', per stirpes' : ''}.
            </Text>
          )}
          {residuaryEstate.distributionType === 'custom' && residuaryEstate.customBeneficiaries?.length > 0 && (
            <View>
          ...
          {residuaryEstate.perStirpes && children.length > 0 && (
            <Text style={styles.paragraph}>
              If any beneficiary named above does not survive me, their share shall pass
        ...
        <Article title="SURVIVORSHIP REQUIREMENT" articleNum={articleNums.survivorship}>
          <Text style={styles.paragraph}>
            No beneficiary shall be entitled to receive any benefit under this Will
            unless such beneficiary survives me by {survivorshipPeriod} days. If a
            beneficiary fails to survive me by such period, such beneficiary shall be
            deemed to have predeceased me for all purposes of this Will.
          </Text>
        </Article>
        ...
        {articleNums.digital && (
          <Article title="DIGITAL ASSETS" articleNum={articleNums.digital}>
            ...
            {digitalAssets.fiduciary && (
              <Text style={styles.paragraph}>
                I specifically authorize {digitalAssets.fiduciary} to serve as my
                digital fiduciary with full power to access my digital accounts and assets.
              </Text>
            )}
          ...
          <Article title="PET CARE PROVISIONS" articleNum={articleNums.pets}>
            {pets.items.map((pet, i) => (
              <View key={i} style={{ marginBottom: 10 }}>
                <Text style={styles.paragraph}>
                  {i + 1}. My {pet.type || 'pet'}{pet.name ? ` named ${pet.name}` : null}:
                </Text>
                  ...
                  funeral.serviceType === 'memorial' ? 'Memorial service' :
                    funeral.serviceType === 'celebration' ? 'Celebration of life' :
                      funeral.serviceType === 'private' ? 'Private family-only' :
                        funeral.serviceType === 'none' ? 'No formal service' : funeral.serviceType}
              </Text>
            )}
            {funeral.location && (
          ...
          {debtsAndTaxes.include ? (
            <View>
              {debtsAndTaxes.paymentOrder === 'residuary' && (
                <Text style={styles.paragraph}>
                  All of my legally enforceable debts, funeral expenses, costs of
                  administration, and any applicable taxes shall be paid from my
                  residuary estate before distribution.
                </Text>
              )}
              {debtsAndTaxes.paymentOrder === 'proportional' && (
                <Text style={styles.paragraph}>
                  All of my legally enforceable debts, funeral expenses, costs of
                  administration, and any applicable taxes shall be paid proportionally
                  from all assets of my estate.
                </Text>
              )}
              {debtsAndTaxes.paymentOrder === 'specific' && debtsAndTaxes.specificInstructions && (
                <Text style={styles.paragraph}>
                  My debts, expenses, and taxes shall be paid as follows: {debtsAndTaxes.specificInstructions}
                </Text>
              )}
            </View>
          ) : (
            <Text style={styles.paragraph}>
              I direct my Personal Representative to pay all of my legally enforceable
              debts, funeral expenses, and costs of administration from my residuary estate.
            </Text>
          )}
        </Article>
        ...
        {articleNums.disinherit && (
          <Article title="DISINHERITANCE" articleNum={articleNums.disinherit}>
            <Text style={styles.paragraph}>
              I have intentionally and with full knowledge of the consequences omitted
              to provide for the following persons who might otherwise claim to be
              entitled to a share of my estate:
            </Text>
            {disinheritance.persons.map((person, i) => (
              <View key={i}>
                <Text style={styles.listItem}>
                  {i + 1}. {person.name}{person.relationship ? ` (${person.relationship})` : null} shall
                  receive no benefit from my estate.
                </Text>
                {person.reason && (
                  <Text style={[styles.listItem, styles.indent]}>
        ...
        {articleNums.noContest && (
          <Article title="NO CONTEST CLAUSE" articleNum={articleNums.noContest}>
            <Text style={styles.paragraph}>
              If any beneficiary under this Will, directly or indirectly, contests or
              attacks this Will or any of its provisions, any share or interest in my
              estate given to that contesting beneficiary under this Will is revoked
              and shall be disposed of as if that contesting beneficiary had
        ...
        <Article title="SIMULTANEOUS DEATH" articleNum={articleNums.simultaneousDeath}>
          <Text style={styles.paragraph}>
            If any beneficiary and I should die simultaneously, or under circumstances
        ...
        <Article title="TAX APPORTIONMENT" articleNum={articleNums.taxApportionment}>
          <Text style={styles.paragraph}>
            All estate, inheritance, succession, and other death taxes payable by reason
            of my death shall be paid from my residuary estate as an expense of
            administration, without apportionment and without reimbursement from any
            recipient of property included in my taxable estate.
          </Text>
        </Article>
        ...
        <ArticleLong title="GENERAL PROVISIONS" articleNum={articleNums.general}>
          ...
          <Text style={styles.paragraph}>
            C. Gender and Number: The masculine, feminine, or neuter gender and the
            singular or plural number shall include the others as context indicates.
          </Text>
          <Text style={styles.paragraph}>
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        <View style={{ marginTop: 60 }}>
          ...
          <Text style={styles.paragraph}>
            ...
            or asked another to sign for them, did so in our presence, and that we
            believe this person to be of sound mind. We have signed this Will as
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        <Text style={[styles.paragraph, { marginBottom: 8 }]}>
          ...
          as the Testator's Last Will and that the Testator signed it willingly, or
          directed another to sign for the Testator, and that each of the witnesses,
          in the presence and at the request of the Testator, signed the Will as
          witness in the Testator's presence and in the presence of each other, and
          that the Testator was at that time eighteen years of age or older, of sound
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 20 }}>
          ...
          <View style={{
            ...
            <Text style={{ fontSize: 9 }}>[NOTARY SEAL]</Text>
          </View>
        </View>
