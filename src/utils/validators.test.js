...
describe('validateChildren', () => {
  ...
  it('does not require guardian for adult children', () => {
    const errors = validateChildren(
      [{ name: 'Adult Child', isMinor: false }],
      { name: '' }
    )
...
describe('validateDistribution', () => {
  it('returns no errors for default spouse distribution', () => {
    const errors = validateDistribution({ distributionType: 'spouse' })
    expect(Object.keys(errors)).toHaveLength(0)
  })

  it('validates split shares total 100', () => {
  ...
  it('accepts split shares totaling 100', () => {
    const errors = validateDistribution({
      distributionType: 'split',
      spouseShare: 60,
      childrenShare: 40
